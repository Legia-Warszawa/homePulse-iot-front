...
class _InteractiveOutsideChartState extends State<InteractiveOutsideChart> {
  ...
  List<LineChartBarData> _barsForTab(int tab) {
    ...
    final spots = list.asMap().entries.map((e) {
      ...
      if (tab == 0) {
        ...
        y = e.value.pressure - 900; // przeskalowane ciśnienie do czytelnej osi
      }
      return FlSpot(x, y);
    }).toList();
  ...
  Widget build(BuildContext context) {
    final data = _sortedOutsideData;
    final theme = Theme.of(context);
    final mq = MediaQuery.of(context);
    final double chartHeight = (mq.size.height * 0.35).clamp(160.0, mq.size.height * 0.55);
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Dane Zewnętrzne', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            SizedBox(height: 8),
            ...
            Container(
              ...
              child: Padding(
                ...
                child: data.isEmpty
                    ? Center(child: Text('Brak danych', style: TextStyle(color: Colors.grey[600])))
                    : LineChart(
                        LineChartData(
                          minX: minX,
                          maxX: maxX,
                              ...
                              getTooltipItems: (touchedSpots) {
                                return touchedSpots.map((spot) {
                                  ...
                                  if (idx >= 0 && idx < data.length) {
                                    final d = data[idx];
                                    if (activeTab == 0) {
                                      return LineTooltipItem('Temp: ${d.temperature.toStringAsFixed(1)}°C\n${d.timestamp!.hour.toString().padLeft(2,'0')}:${d.timestamp!.minute.toString().padLeft(2,'0')}', TextStyle(color: Colors.white));
                                    } else if (activeTab == 1) {
                                      return LineTooltipItem('Wilgotność: ${d.humidity.toStringAsFixed(1)}%\n${d.timestamp!.hour.toString().padLeft(2,'0')}:${d.timestamp!.minute.toString().padLeft(2,'0')}', TextStyle(color: Colors.white));
                                    } else {
                                      return LineTooltipItem('Ciśnienie: ${d.pressure.toStringAsFixed(0)} hPa\n${d.timestamp!.hour.toString().padLeft(2,'0')}:${d.timestamp!.minute.toString().padLeft(2,'0')}', TextStyle(color: Colors.white));
                                    }
                                  }
                                  return null;
                            ...
                            leftTitles: AxisTitles(
                              sideTitles: SideTitles(
                                ...
                                getTitlesWidget: (value, meta) {
                                  if (activeTab == 0) {
                                    return Padding(padding: EdgeInsets.only(right:6), child: Text('${value.toInt()}°C', style: TextStyle(fontSize: 10)));
                                  } else if (activeTab == 1) {
                                    return Padding(padding: EdgeInsets.only(right:6), child: Text('${value.toInt()}%', style: TextStyle(fontSize: 10)));
                                  } else {
                                    return Padding(padding: EdgeInsets.only(right:6), child: Text('${(value + 900).toInt()}', style: TextStyle(fontSize: 10)));
                                  }
                                },
                              ),
                            ),
                                ...
                                getTitlesWidget: (value, meta) {
                                  ...
                                  if (idx >= 0 && idx < data.length) {
                                    ...
                                    return Padding(
                                      ...
                                      child: Transform.rotate(angle: -0.3, child: Text('${ts.hour.toString().padLeft(2,'0')}:${ts.minute.toString().padLeft(2,'0')}', style: TextStyle(fontSize: 9))),
                                    );
                                  }
                                  return Text('');
                                },
            ...
            Row(
              ...
              children: [
                _buildLegendItem(
                  activeTab == 0 ? 'Temperatura (°C)' : activeTab == 1 ? 'Wilgotność (%)' : 'Ciśnienie (hPa)',
                  activeTab == 0 ? Colors.red : activeTab == 1 ? Colors.green : Colors.purple,
                ),
              ],
            ),
